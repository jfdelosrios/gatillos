...
int periodoMedia = 750;
int Shift = 0;
int MAtype = MODE_LWMA;
int ApplyToPrice = PRICE_CLOSE;
int VShift = 0;

int distanciaMedia1 = 74;
double fraccionPenetrando = 0.4;
...
bool climaxArriba(const int index)
{
	...
	double cuerpo;
	double mechaInferior;
	double mechaSuperior;

	if (Close(index) > Open(index))
	{
		cuerpo = Close(index) - Open(index);
		mechaInferior = Open(index) - Low(index);
		mechaSuperior = High(index) - Close(index);
	}
	else
	{
		cuerpo = Open(index) - Close(index);
		mechaInferior = Close(index) - Low(index);
		mechaSuperior = High(index) - Open(index);
	}

	if (cuerpo >= (0.625*mechaInferior))
		return false;
	
	if (mechaSuperior >= (0.461539*mechaInferior))
		return false;
...
bool climaxAbajo(const int index)
{
	...
	if (cuerpo >= (0.625*mechaSuperior))
		return false;

	if (mechaInferior >= (0.461539*mechaSuperior))
		return false;
...
short int internaAbajo(const int index)
{
	...
	*/
	...
	if ((Open(index + 1) - Close(index + 1)) <= (0.5*(High(index + 1) - Low(index + 1)))) //por jf
		return 1;
...
short int internaArriba(const int index)
{
	...
	if ((Close(index + 1) - Open(index + 1)) <= (0.5*(High(index + 1) - Low(index + 1)))) //por jf
		return 1;
...
short int externaArriba(const int index)
{
	...
	*/
	...
	if (Close(index + 1) < Open(index))
		if ((Open(index + 1) - Open(index)) >= (0.5*(Open(index + 1) - Close(index + 1)))) //por jf, cuerpo muy por fuera
			return 0;

	if ((Low(index + 1) <= Low(index)) || (High(index + 1) >= High(index))) //por jf
		return 1;

	if ((Close(index) - Open(index)) <= (0.5*(High(index) - Low(index)))) //por jf
		return 1;
...
short int externaAbajo(const int index)
{
	...
	if (Close(index + 1) > Open(index))
		if ((Open(index) - Open(index + 1)) >= (0.5*(Close(index + 1) - Open(index + 1)))) //por jf, cuerpo muy por fuera
			return 0;

	if ((Low(index + 1) <= Low(index)) || (High(index + 1) >= High(index))) //por jf
		return 1;

	if ((Open(index) - Close(index)) <= (0.5*(High(index) - Low(index)))) //por jf
		return 1;
...
void DetectaVelaImperante(const int index)
{
	...
	const double maximoIndex = max(Open(index), Close(index));
	const double maximoCont = max(Open(cont), Close(cont));
	const double minimoIndex = min(Open(index), Close(index));
	const double minimoCont = min(Open(cont), Close(cont));

	if (High(index) > High(cont)) {
		//if (maximoIndex > maximoCont) {
		velaImperante[index] = valorVelaImperante;
		return;
	}

	if (Low(index) < Low(cont)) {
		//if (minimoIndex < minimoCont) {
		velaImperante[index] = valorVelaImperante;
		return;
	}

	if ((High(index) == High(cont)) && (Low(index) == Low(cont)))
		//if ((maximoIndex == maximoCont) && (minimoIndex == minimoCont))
	{
		velaImperante[index] = valorVelaImperante;
		return;
	}
...
void DetectaVelaImperanteOriginal(const int index)
{
	if (index == (Bars() - 1))
		velaImperante[index] = valorVelaImperante;
	...
	*/
	...
	if (MA.GetValue(index + 1) <= MA.GetValue(index))
		...
		case 1:
			...
			if (min(Low(index), Low(index + 1)) > (valorMedia + distanciaMedia1 * Point()))
				break;
	...
	if (MA.GetValue(index + 1) >= MA.GetValue(index))
		...
		case 1:
			...
			if (max(High(index), High(index + 1)) < (valorMedia - distanciaMedia1 * Point()))
				break;
	...
	/*
	if (TimeToStr(Time(index)) == "2003.08.11 09:00:00")
		Print("t");
*/
}